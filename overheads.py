...
def read_overheads(file_path):
    with file_path.open(mode='r', encoding='UTF-8', newline="") as file:
        ...
        for row in reader:
            #type of expenses
            expense_type = row[0]
            #percentage of each type out of 100%, with decimals
            amount = float(row[1])
            #allocate the names to the rows 
            overHead.append((expense_type, amount))

        return overHead

def analyze_highest_overhead(overheads):
    #viarable starting at 0, coded to store the highest number comparing to the previous loop
    highest_amount = 0
